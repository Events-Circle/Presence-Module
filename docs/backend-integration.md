...
// Library usage example only. The Expo app uses mobile/service.ts and SecureStore.
...
The Expo UI clears snapshots and editors on organization/account changes and discards stale snapshot results using a generation counter. In-flight requests are not cancelled; stale results are not installed into the next organization's dashboard. No automatic mutation retry or silent account switching is provided.
...
Uploads go to Core as multipart field `file`; let FormData set its boundary. Do not upload directly to the bucket or pass object keys. Core accepts JPEG/PNG/WebP up to 5 MB, processes to WebP and returns a READY media ID. Store and attach that ID. The native file-picker/FormData adapter is now implemented in mobile/service.ts. Physical-device upload testing remains required on Android and iOS.
...
Backend currently lacks a public frontend URL, so share-link generation is unavailable until that website exists. The Expo frontend now includes native SecureStore persistence, navigation and an EAS preview build profile. CORS for a browser host and EAS account/project linking remain pending.
