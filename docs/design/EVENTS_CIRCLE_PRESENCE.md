...
BrandIntro.tsx uses a 3.43-second opening sequence. Individual outlines overlap with smooth color fill (750–1050 ms); the symbol settles with smoothstep easing from 1000–1400 ms. Events is brush-drawn from 1400 ms, followed by CIRCLE from 2300 ms. Neighboring letters overlap slightly. The complete original artwork appears at 2980 ms, holds until 3190 ms and fades for 240 ms.

`brushStrokes.ts` contains hand-directed pen trajectories used only as reveal masks. Rounded white strokes progressively uncover the exact original colored artwork; they do not replace the lettering with a font or redraw its visible geometry. Separate pen lifts are timed by their measured lengths. Existing join-aware clipping preserves the connected cursive shape. All logo space remains reserved. The final hold uses the unmasked original artwork, retaining the supplied proportions and colors.
